# coding: utf-8
from unifi.basedevice import BaseDevice
from . import basecommand 
import json
import re
import psutil
from . import utils
from . import pfsense_utils
from . import ip_calculator
import time
import _thread
import logging
...
class UnifiUSGPro(BaseDevice):
    ...
    def cfgversion(self):
        if 'mgmt_cfg' in self.config and 'cfgversion' in self.config['mgmt_cfg']:
    ...
    def getCurrentMessageType(self):
        if ('gateway' in self.config 
            ...
            and ('key' not in self.config['gateway'] or self.config['gateway']['key']=="" )):
            return -1     
        if ('gateway' in self.config 
            and not self.config['gateway']['is_adopted'] 
            and 'key' in self.config['gateway'] 
            and not self.config['gateway']['key']=="") : #discover
            return 1     
        if 'gateway' in self.config and self.config['gateway']['is_adopted'] : #info
            return 2     
    ...
    def getHostname(self):
        if 'host' in self.config['gateway'] :
    ...
    def create_if_element(self,interface,if_stats,io_counters,if_addrs,dpingerStatuses,hostsstatus):
        ...
        dpinger = dpingerStatuses[name] if 'name' in dpingerStatuses else None
        isup = stat.isup
        
        if "pfsense-ppp" in interface and interface["pfsense-ppp"] is not None:
            ipv4 = utils.get_ipv4addr(if_addrs,interface["pfsense-ppp"])
            dpinger = dpingerStatuses[interface["pfsense-ppp"]] if interface["pfsense-ppp"] in dpingerStatuses else dpinger
            isup = if_stats[interface["pfsense-ppp"]].isup if interface["pfsense-ppp"] in if_stats else False
        
        ipaddress1 = ipv4.address if ipv4 is not None else "0.0.0.0"
        ntopstat = hostsstatus[ipaddress1] if hostsstatus is not None and  ipaddress1 in hostsstatus else None
        ...
        data = {
                ...
                "up": isup ,
                "uptime": ntopstat['duration'] if ntopstat is not None and 'duration' in ntopstat else 0
    ...
    def create_network_table_element(self,interface,if_stats,io_counters,if_addrs,dpingerStatuses,hostsstatus):
        ...
        if ('ntopng' in self.config
            and 'enabled' in self.config['ntopng']
            and self.config['ntopng']['enabled']
            and 'showhosts' in self.config['ntopng']
            ...
            if(hostsstatus is not None):
                hosts = [host for key,host in list(hostsstatus.items()) if host['local_network_name'] == netname and not host['is_broadcast'] and not host['is_multicast']]
                data['host_table']=[]
                for rhost in hosts:
                    data['host_table'].append(self.create_host_table_element(rhost))
                
        if (interface["wan"] or  ( 'address' in interface and 'dhcp' in interface['address'])) and ipv4 is not None:
    ...
    def appendExtraInformMessage(self,data):
        ...
        if ('ntopng' in self.config 
            and 'enabled' in self.config['ntopng'] 
            and self.config['ntopng']['enabled']  
            and 'user' in self.config['ntopng'] 
            and 'password' in self.config['ntopng']
            and 'url' in self.config['ntopng']
    ...
    def process_or_create_if(self,key,data):
        ...
        if(len(tmp)>0):
            ...
            local={
                    'unifi':key,
                    'unifi-description':data['description'] if 'description' in data else "LAN",
                    'pfsense':'',
                    'enabled':True,
                    'wan':False
                }
        ...
        if(local is not None):
            ...
            local['pppoe']='pppoe' in data
            local['enabled']='disable' not in data 
    ...
    def process_command(self,data):
      if(data['cmd']=='speed-test'):
          ...
          self.save_config()
          self.reload_config()
          _thread.start_new_thread( self.speedtest_check ,() )
        ...
        if result['_type'] == 'setparam':
            for key, value in list(result.items()):
                if key not in ['_type', 'server_time_in_utc', 'mgmt_cfg','system_cfg']:
                    ...
                    if system_cfg["system"] is not None and "host-name" in system_cfg["system"]:
