...
class BaseDevice:
    def __init__(self,device="",type="",configfile=""):
        ...
        self.lastError = "None"
        self.firmware = self.config.get('gateway').get('firmware')
        self.device = device
        self.type = type
        self.state=DS_READY
        ...
        if('gateway' in self.config and 'lan_if' in self.config['gateway']):
    ...
    def _send_inform(self, data,usecbc):
        ...
        try:
            request = urllib.request.Request(url, cryptoutils.encode_inform(self.getKey(),data,usecbc,self.mac), headers)
    ...
    def _send_stun(self):
        try:
            if 'mgmt_cfg' in self.config and 'stun_url' in self.config['mgmt_cfg']:
